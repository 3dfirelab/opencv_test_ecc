...
def align(templateMask):
    ...
    criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 5000,  1e-4)
    ...
    for i in range(0,2) :
        (cc, warp_matrix) = cv2.findTransformECC (get_gradient(im_color[:,:,2]), get_gradient(im_color[:,:,i]),warp_matrix, warp_mode, criteria,
                                                  ...
                                                  inputMask = np.ones_like(im_color[:,:,2]),
...
if __name__ == '__main__':
    ...
    im_color_ = cv2.resize(im_color,(255,217),cv2.INTER_AREA)
    im_color = im_color_
    ...
    mask[jjb:jje,iib:iie] = 1
   

    #original example, template has most of the feature of input image
    templateMask = None
    im_aligned = align(templateMask)
    image = cv2.cvtColor(im_aligned, cv2.COLOR_BGR2RGB)
    plt.imshow(image)
    plt.title('original')
    plt.show()
    
    #apply mask to red band, the template image of findEccTransfrom
    im_color[:,:,2] = np.where(mask==0, 255, im_color[:,:,2])
    templateMask = None
    im_aligned = align(templateMask)
    image = cv2.cvtColor(im_aligned, cv2.COLOR_BGR2RGB)
    plt.imshow(image)
    plt.title('alignement with small template')
    plt.show()
    ...
    image = cv2.cvtColor(im_aligned, cv2.COLOR_BGR2RGB)
    plt.imshow(image)
    plt.title('alignement with small template + mask')
    plt.show()
